...
def get_wrapper_path(scripts_directory: Path):
    r"""
    Return the paths of the resources located in "python\Scripts".

    This function is either run from the "python\Scripts" directory or from the original source directory. In the case
    of the original directory, we want to copy over all the necessary resources to overwrite the default python
    `.exe` wrappers.
    """
...
def clean_up_scripts_directory(scripts_directory: Path):
    r"""
    This function ensures that all the scripts in "python\Scripts" are made to run after the distribution has changed
    directories. This involves replacing the original binaries with alternative wrappers and replacing the pip launcher
    with an alternative to ensure this cleanup routine is run after ever pip call.
    """
